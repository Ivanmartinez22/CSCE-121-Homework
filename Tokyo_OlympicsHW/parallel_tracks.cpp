...
// Valid char check 
bool zero_to_nine_check(char chr){
	bool return_value;
	if(chr >= 48 && chr <= 57){
		return_value = true;
	}
...
bool zero_to_nine_float_check(char chr){
	bool return_value;
...
bool name_char_check(char chr){
	bool return_value;
	if((chr >= 97 && chr <= 122) || (chr == 32) || (chr >= 65 && chr <= 90)){
		return_value = true;
	}
	else{return_value = false;}
	return return_value;
...
bool is_valid_number(const std::string& str) {
	...
	while(counter != length){
		counter++;
		char i = str[counter - 1];
		if(zero_to_nine_check(i) == false){
			return_value = false;
			break;
		}
	}
...
bool is_valid_time(const std::string& str) {
	...
	while(counter != length){
		...
		if(zero_to_nine_float_check(i) == false){
			return_value = false;
			break;
		}
	}
...
//---------------------------------------------------------
void get_runner_data( const std::string& file, float *timeArray, std::string* countryArray,unsigned int* numberArray, std::string *lastnameArray){
